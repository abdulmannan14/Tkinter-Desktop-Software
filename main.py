...
def leble():
    qs4 = tk.Label(root, text='The Final Output value is:')
    qs4.config(font=('helvetica', 20), bg='#043C74', fg='white')
    canvas1.create_window(730, 500, window=qs4)
    token=''
    qs5 = tk.Label(root, text=token)
    qs5.config(font=('helvetica', 20), bg='#043C74', fg='white')
    canvas1.create_window(730, 550, window=qs5)
    b='hello'
            ...
            if thresholdentry.get():
                if len(lista) != 0:
                    ...
                    while int(token) > int(threshold):
                        ...
                        if int(token) > int(threshold):
                            token = int(token) - int(total_of_lista)
                    qs4 = tk.Label(root, text='The Final Output value is:')
                    qs4.config(font=('helvetica', 20), bg='#043C74', fg='white')
                    canvas1.create_window(730, 500, window=qs4)
                    qs5 = tk.Label(root, text=token)
...
start = tk.Button(root, text ="Start", command = leble,bg='#043C74',fg='white')
